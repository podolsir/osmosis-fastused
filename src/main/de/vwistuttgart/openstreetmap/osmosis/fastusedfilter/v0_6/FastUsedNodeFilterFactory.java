...
/**
 * Extends the basic task manager factory functionality with fast-used-node
 * filter task specific common methods.
 * 
 * @author Igor Podolskiy
 */
public class FastUsedNodeFilterFactory extends TaskManagerFactory {
	...
	protected TaskManager createTaskManagerImpl(TaskConfiguration taskConfig) {
		int bufferCapacity = getIntegerArgument(taskConfig, 
				ARG_BUFFER_CAPACITY, DEFAULT_BUFFER_CAPACITY);
		IdTrackerType idTrackerType = getIdTrackerType(taskConfig);
		return new MultiSinkRunnableSourceManager(
			taskConfig.getId(),
			new FastUsedNodeFilter(idTrackerType, bufferCapacity),
