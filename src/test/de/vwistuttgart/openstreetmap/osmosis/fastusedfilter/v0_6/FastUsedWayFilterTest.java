...
public class FastUsedWayFilterTest {

	@Test
	public void standard() throws Exception {
		File inFile = TestUtil.makeDataFile("/data/input/v0_6/inputRelationWay.xml");
		...
		try {
			SinkEntityInspector result = TestUtil.runFastUsedFilter(new FastUsedWayFilter(IdTrackerType.IdList, 1),
					reader1, reader2, new AcceptAllFilter());
			TestUtil.assertCount(1 + 1 + 2 + 6, result.getProcessedEntities());
			...
			TestUtil.assertEntitiesCountByType(6, EntityType.Node, result.getProcessedEntities());
		} finally {
			inFile.delete();
		}
	}
	...
	public void waysEmpty() throws Exception {
		...
		try {
			...
			TestUtil.assertCount(1 + 1 + 0 + 6, result.getProcessedEntities());
			TestUtil.assertEntitiesCountByType(1, EntityType.Bound, result.getProcessedEntities());
			TestUtil.assertEntitiesCountByType(1, EntityType.Relation, result.getProcessedEntities());
			TestUtil.assertEntitiesCountByType(0, EntityType.Way, result.getProcessedEntities());
			TestUtil.assertEntitiesCountByType(6, EntityType.Node, result.getProcessedEntities());
		} finally {
			inFile.delete();
		}
	...
	@Test
	public void relationsEmpty() throws Exception {
